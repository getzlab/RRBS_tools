...
if [ $# -eq 0 ]; then
    echo "Usage: $0 -1 FQ1  -s SAMPLE -g GENOME -r REFERENCE_FA -z SIZE [-q seed_size -c C_THREADS -n -a -o -x]"
    echo ""
...
## parameters
while getopts 1:i:s:g:c:m:r:q:z:naox option
do
    case "${option}"
        in
	1) FQ1_INPUT=${OPTARG};;
...
## ----------------------------------------------------------------
results_dir=${working_dir}/${SAMPLE}_${GENOME}_SR
...
if [ $CUTADAPT  ]; then  
    ...
    echo current adaptor sequence: ${adaptors[illumina]}
...
echo "SR Bisulfite Pipeline Complete"
