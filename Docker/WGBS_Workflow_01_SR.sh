...
if [ $# -eq 0 ]; then
  echo "Usage: $0 -1 FQ1 -2 FQ2 -s SAMPLE -g GENOME [-c C_THREADS -n -a -o]"
  echo ""
...
## parameters
while getopts 1:2:i:s:g:c:nao option
do
    case "${option}"
        in
        1) FQ1_INPUT=${OPTARG};;
...
function trim_galore {
    echo "TimGalore clipping incl RRBS alternative to Nugen's scripts - may run multi-threaded."
}
...
## ----------------------------------------------------------------
results_dir=${working_dir}/wgbs_${SAMPLE}_${GENOME}_SR-dedup_${date_flag}
...
bio_muelltonne+=($fq1_trimmed .log_a .log_b)
...
$bsmap \
  ...
  -d ${genome_index[$GENOME]} \
  -a $FQ1 \
  | $samtools sort -m 8G --threads $p_threads --output-fmt BAM -o $bam_file -

print_str "Creating BAM Index for $bam_file"
$samtools index -@ $((2*$p_threads)) $bam_file

print_str "Creating BAM Statistcis $bam_file"
$bamstats --bam $bam_file --threads $((2*$p_threads)) --verbose | tee ${bam_file}.stats.txt
...
$bamstats --bam $bam_dedup --threads $((2*$p_threads)) --verbose | tee ${bam_dedup}.stats.txt
...
echo "***********************************************************"
echo "***     TESTING"
echo "***     SR Workflow ** NOT YET TESTED "
echo "***********************************************************"
echo ""
